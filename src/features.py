...
def daily_returns(prices: pd.DataFrame, price_col: str = "adjClose") -> pd.DataFrame:
    ...
    Keep the output in long form so downstream portfolio code can choose when
    and how to pivot. Returns are computed within each ticker before any
    cross-asset calendar merge.
    """
    ...
    out["return"] = out.groupby("ticker")[price_col].pct_change(fill_method=None)
    return out
